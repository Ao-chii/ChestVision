...
# 测试集transform - DacNet和Baseline使用不同的transform！
# DacNet: Resize(256) + CenterCrop(224)
transform_dacnet = transforms.Compose([
...
def load_model_and_predict(checkpoint_dir, model_name, transform):
    ...
    # Create dataset and dataloader with model-specific transform
    print(f"Using transform: {[t.__class__.__name__ for t in transform.transforms]}")
    test_dataset = ChestXrayDataset(test_df, images_dir, transform=transform)
...
# Load predictions from both models with their respective transforms
